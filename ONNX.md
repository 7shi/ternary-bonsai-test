# Investigation: Why DirectML Output Quality Differs Between model_fp8 and model_q2_to_fp8

## Summary

`model_q2_to_fp8` and `model_fp8` store weights as dense fp8 tensors via the same `Cast` + `MatMul` pattern, so they should theoretically produce identical results. However, `model_q2_to_fp8` produces garbled English token sequences under DirectML while `model_fp8` does not. The root cause is not the weight representation but the **ONNX graph structure**: `model_q2_to_fp8` inherits the q2 source graph intact, which relies on `com.microsoft` custom ops that DirectML does not fully support.
...
The same graph structure is also inherited by `model_q2_to_q4` and `model_q2_to_q8`, which explains why all three q2-derived models exhibit garbled output under DirectML.
...
The same mechanism affects `model_q2_to_q8` and `model_q2_to_q4` since neither conversion script touches the attention or normalization nodes.

---

## Why CPU Is Unaffected

CPUExecutionProvider has well-tested reference kernels for `GroupQueryAttention` and the other `com.microsoft` ops. That is why `model_q2_to_fp8 / CPU`, `model_q2_to_q8 / CPU`, and `model_q2_to_q4 / CPU` all produce correct Japanese output.

---

## Feasibility of Converting the Graph to Standard Ops

Converting `model_q2_to_fp8` to use only standard ONNX ops (to match `model_fp8`) is technically possible but has varying difficulty per op:
...
The high cost of expanding `GroupQueryAttention` makes this approach impractical compared to simply using `model_fp8` (generated from safetensors via `optimum-cli export onnx`) when DirectML + fp8 quality is required.
...
3. **All q2-derived models** (`model_q2_to_fp8`, `model_q2_to_q4`, `model_q2_to_q8`) share this limitation because none of the conversion scripts rewrite the attention or normalization nodes.
4. **For DirectML with reliable quality**, use `model_fp16` or `model_fp8` (safetensors-derived, standard ONNX ops only).
5. **For CPU with minimum model size**, `model_q2_to_q4 / CPU` remains the best trade-off: fast load, fast generation, correct output, and benefits from OS file cache on repeated runs.
